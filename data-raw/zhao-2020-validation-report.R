...
zhao_validation <- function(input = "data-raw/zhao-2020-table-simulations",
  ...
  lines <- c("# Validation against Zhao et al. (2020) simulations", "",
    ...
    "**Log-logistic mismatch:** Table 1 specifies S(t) = 1 / [1 + (lambda*t)^(1/gamma)], lambda = exp(-4), gamma = 0.5. An inverse draw is `exp(4) * (U^(-1) - 1)^0.5`, with mean exp(4)*pi/2, approximately 85.76. The current source instead uses `exp(-4) * (U^(-0.5) - 1)^0.5`, with mean exp(-4)*pi/4, approximately 0.01438. Both the scale and inverse transform differ. This can generate vastly more events, and those cells cannot validate Zhao's design. The report leaves the simulation code and existing outputs unchanged; log-logistic results, if present, are labeled design mismatch.", "",
    ...
    "Task IDs 4, 8, 12, and so on select log-logistic gaps. Under the current incorrect draw, a renewal approximation gives roughly 1.37, 2.05, and 3.35 million completed events per null dataset at low, medium, and high heterogeneity: 200*90*E(1/Z)/E(base gap), with E(1/Z)=log(b/a)/(b-a). Almost all continuous event times are distinct. Each dense 200-by-event-count double matrix therefore occupies about 2.05, 3.05, or 4.98 GiB. The rank test retains roughly six such matrices, with additional temporary arrays. Across four workers, those six matrices alone account for approximately 49, 73, or 120 GiB. These are approximate typical null-design allocations, not guaranteed peak bounds. A 25 GiB allocation is consequently insufficient for the current generator. The nested risk-set calculations also become extremely slow.", "",
    "A separate corrected-generator sizing pilot (`data-raw/zhao-2020-memory-pilot.R`) draws five null datasets at each heterogeneity level in an isolated R environment. The September 23, 2026 local macOS run produced 222–676 completed events per dataset and a maximum resident set size of 196,788,224 bytes (about 188 MiB) for one R process. This is a small pilot, not a cluster or full-run peak measurement. After correcting and verifying the production generator, retaining 16–25 GiB total per four-worker task is a conservative starting allocation; measure cluster MaxRSS on a short pilot before scaling up. Increasing memory for the incorrect generator would not make its results a validation of Zhao's simulations.", "",
  ...
  lines <- c(lines, "## Next steps implied by this validation", "",
    ...
    "2. Correct and independently verify the log-logistic inverse transform before rerunning those cells; keep corrected runs separate from this batch with recorded source provenance.",
