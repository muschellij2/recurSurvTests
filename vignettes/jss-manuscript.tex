...
rank tests, (ii) the Peña-Strawderman-Hollander generalized
...
simulation designs. We provide a comparison to published tables and also
show an example similar to data analysis from polysomnography sleep data
analyzing sleep bouts.
...
example, the Andersen-Gill \citep{andersen1982},
Prentice-Williams-Peterson (PWP) \citep{prentice1981}, and related
Cox-model formulations provide score, Wald, and likelihood-based
inference for regression coefficients; mixed-effects and frailty
extensions are available in \texttt{coxme} \citep{coxme} and
\texttt{frailtypack} \citep{frailtypack}. These are important tools when
a conditional covariate or frailty effect is the scientific target. They
do not, however, supply a nonparametric global test for equality of an
estimated recurrent gap-time survival curve or tests contrasting such
survival curves across groups. Existing recurrent-curve R software has
also made important contributions: \texttt{survrec} \citep{survrec}
includes Wang-Chang and Peña-Strawderman-Hollander estimators, and
\texttt{newTestSurvRec} \citep{newtestsurvrec} includes recurrent-event
curve estimators and rank-test procedures. Their available estimators
and testing paths do not all target the same curve, so a test cannot be
transferred between them solely on the basis of a shared label such as
``recurrent survival.''
...
\subsubsection{\texorpdfstring{Relationship to
\texttt{newTestSurvRec::Dif.Surv.Rec()}}{Relationship to newTestSurvRec::Dif.Surv.Rec()}}\label{relationship-to-newtestsurvrecdif.surv.rec}

The existence of \texttt{newTestSurvRec::Dif.Surv.Rec()} does not make
the Zhao and Luo-Huang implementations redundant, but the functions
should not be treated as interchangeable. A source-level inspection of
\texttt{Dif.Surv.Rec()} shows the following distinctions:
...
\texttt{newTestSurvRec::Dif.Surv.Rec()}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Zhao et al.~(2020) / \texttt{zhao\_rank\_test()}
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Curve/risk-set path & The function calls
\texttt{survfitr(...,\ type\ =\ "pe")}, i.e.~its displayed test path is
built from the Peña-Strawderman-Hollander/GPLE-style pooled gap-record
fit. The package also contains a separate Wang-Chang fitter, but that is
not the estimator selected inside this function. & Uses Wang-Chang
weighted risk and event masses, with each subject's eligible gaps scaled
by \(1/m_i^*\). \\
Score numerator & At each pooled event time, group-1 events minus the
pooled-risk expected group-1 count. & The same broad
observed-minus-expected rank-test idea, but with the Wang-Chang weighted
counts and the score in Equation 3. \\
Weight families & Provides a broad family (\texttt{LRrec},
\texttt{Grec}, \texttt{TWrec}, \texttt{PPrec}, \texttt{PMrec},
\texttt{PPrrec}, \texttt{HFrec}, \texttt{CMrec}, \texttt{Mrec}). For
example, \texttt{Grec} uses the total risk-set size and \texttt{TWrec}
its square root. & Implements the three paper weights LR, GB, and PP. In
the paper's notation these are \(1\), \(n/Y^*(t)\), and the pooled
Wang-Chang survival estimate \(S^*(t)\), respectively. Thus the label
``GB'' is not evidence that the \texttt{Grec} calculation is the same
test. \\
Variance & Uses an event-time variance term based on pooled and group
risk-set/event counts, then sums weighted contributions. It does not use
Zhao's subject residual vector. & Uses subject-level residuals from
Equation 6 and the robust variance
\(\widehat V^* = \sum_i \hat\epsilon_i^{*2}\) in Equation 7. The
pooled-risk and group-specific Equation 6 conventions are retained
separately here. \\
Output interpretation & A family of recurrent-curve tests whose default
curve path is PSH/GPLE-style. & A test of equality of Wang-Chang
marginal recurrent gap-time survival distributions, with a
paper-specific robust variance construction. \\
\end{longtable}
}

Consequently, \texttt{Dif.Surv.Rec(type\ =\ "LRrec")} is a useful
historical recurrent curve rank test and an important reference
comparison, but it is not an exact implementation of Zhao's Equation
3--7 construction. In particular, matching the word ``log-rank'' does
not resolve the curve estimator, event weighting, or within-subject
variance choice. The manuscript's numerical checks therefore compare the
curve estimators separately and do not use \texttt{Dif.Surv.Rec()} as a
validation oracle for \texttt{zhao\_rank\_test()}.
...
``log-rank tests.'' In this manuscript, CIF means cumulative-incidence
function, and RCIF means recurrent cumulative-incidence function. METRON
is the journal \emph{METRON}, where the Sivadasan-Sankaran recurrent-CIF
test was published; that procedure is a weighted contrast/Wald-type
procedure and is not a log-rank test.
...
per observed gap. If an \texttt{episode} column is supplied, the package
sorts rows within subject by that column; without it, the existing
within-subject row order is used. Completed gaps have \(\delta_{ij}=1\);
the final row is the terminal right-censored gap with \(\delta_{ij}=0\).
The observed gap duration is \(Y_{ij}\). An optional \texttt{episode}
column makes the ordering explicit. This convention permits a
participant with no completed recurrence to contribute a single terminal
censored gap.
...
survival}\label{wang-chang-marginal-survival}
...
Kaplan-Meier}\label{psh-and-pooled-kaplan-meier}

The Peña-Strawderman-Hollander (PSH) generalized product-limit estimator
\citep{pena2001} uses all observed gap records with unit weight:
...
distribution of an exchangeable/IID inter-occurrence time under a
renewal model (with independent censoring), or when a pooled gap-record
curve is being shown explicitly as a descriptive event-weighted
quantity. It is not a general replacement for Wang-Chang: subject
frailty, dependence of later gaps on earlier gaps, or informative event
counts causes participants with more events to contribute more gap
records and changes the target. In those settings, \texttt{wc\_surv()}
is preferable for a marginal gap-time survival target, with
subject-level bootstrap inference. PSH can still be reported as a
sensitivity curve, but it should be labeled as the pooled/renewal
estimand rather than as a generic recurrent Kaplan-Meier estimate. The
ordinary \texttt{survival::survfit()} curve on the same long records is
numerically the same pooled product-limit calculation; PSH's value is
the recurrent-event interpretation and its explicit renewal assumptions,
not a different product-limit formula.
...
of the complete history. The \texttt{ci\_method} argument applies to the
recurrent-survival curve CI functions (\texttt{recurrent\_surv\_ci()}
and the corresponding curve-contrast workflow): \texttt{"bootstrap"}
gives subject-bootstrap percentile intervals or max-\(t\) bands, whereas
\texttt{"normal"} gives the legacy independence-style pointwise
approximation. The default percentile intervals are pointwise: at a
pre-specified time \(t\), they target \(P\{S(t)\in CI(t)\}=1-\alpha\).
They are the conventional uncertainty summary for a plotted curve, but
do not support a joint claim about every point of the curve. For that
purpose, \texttt{interval\ =\ "simultaneous"} uses a subject-bootstrap
max-\(t\) critical value to form a band over an explicitly supplied
finite time grid. The associated coverage simulation estimates pointwise
and joint coverage separately; its results remain pending until the
configured run has completed.
...
A pointwise 95\% interval has approximately 95\% coverage for \(S(t_0)\)
at one pre-specified gap time \(t_0\). Drawing such intervals at
multiple times does not yield 95\% joint coverage of the full curve. The
simultaneous max-\(t\) band instead targets
...
standard-error calculation on the same long records, and should not be
relied on for dependent recurrent gaps or informative event counts,
because the displayed variance does not account for subject-level
clustering. In contrast, \texttt{ci\_method\ =\ "bootstrap"} provides a
whole-curve subject-bootstrap workflow: percentile intervals for fixed
times or a max-\(t\) band for a finite grid. The resampling itself is
conceptually straightforward; its value here is a uniform analyst-facing
tool that makes the resampling unit, estimator, time grid, and coverage
claim explicit and consistent across WC and PSH analyses. For a more
complex bootstrap procedure (e.g.~nested), users must bootstrap the data
explicitly and then fit the models.
...
\pandocbounded{\includegraphics[keepaspectratio]{jss-manuscript_files/figure-pdf/fig-curves-1.pdf}}
...
gap-time log-rank analogue. Its implementation uses
\(w(t)=\widehat S_{WC}(t-)^\rho\), where the pooled Wang-Chang survival
estimate is evaluated just before the event time. Thus \(\rho=1\)
downweights late events in proportion to the estimated survival
remaining, a Peto-Prentice-type weight; it is not the same as the
log-rank weight \(w(t)=1\), which gives every event time equal weight.
The variance is built from subject-level contributions to the score:
each subject's observed-minus-expected weighted event increments are
summed across that subject's gaps, preserving their covariance rather
than treating the gap rows as independent.
...
by the simulations below; current results are diagnostics of this
implementation and do not establish byte-for-byte reproduction of the
published tables. The group-risk denominator printed in Zhao et al.'s
Equation 6 is retained as the explicit \texttt{zhao\_eq6}
sensitivity/reproduction path. Neither function should be described as
an exact substitute for the other.

There is an unresolved implementation ambiguity in the published
description. Equation 6 appears to define the residual denominator using
the risk set of the subject's own group, whereas the pooled-risk
construction uses the pooled weighted risk set. These choices produce
different subject residuals and hence different robust variance
estimates even when the score numerator is unchanged. The package
therefore labels them separately
(\texttt{variance\_method\ =\ "pooled\_risk"} and \texttt{"zhao\_eq6"})
and does not silently treat one as the exact published convention. We
have drafted a clarification request to the authors asking which
denominator and finite-sample scaling were used in the published
simulations; until that is resolved, the comparisons below are
implementation diagnostics rather than a claim of exact reproduction.

For two paired visits per participant,
\texttt{wc\_paired\_permutation()} randomly swaps visit labels within
participant pairs and recalculates \(G_\rho^*\). Its p-value tests the
within-pair label-exchangeability null. The independent-arm analytical
p-value from \texttt{wc\_logrank()} is returned for context but is not a
paired analysis. This is the package's permutation-based two-group
procedure; the K-group extension currently has an analytical score test
(\texttt{wc\_logrank\_k()}), not a general K-group label-permutation
wrapper.

\subsection{Recurrent competing risks}\label{recurrent-competing-risks}

\texttt{ss\_rcif()} implements recurrent cause-specific
cumulative-incidence functions (RCIFs) following \citet{sivadasan2023}.
The default shared weighting uses the same subject/event-count
denominator for the pooled and cause-specific processes, so the
cause-specific increments add to the pooled increment. The 2023 paper's
printed Equation 15 instead uses a cause-specific \(m_{il}\) denominator
and an indicator for subjects with at least two events of that cause.
Taken literally, that expression can make the cause-specific increments
nonadditive. The \texttt{cause\_weighting\ =\ "literal\_eq15"} option is
retained for sensitivity and formula-reproduction purposes; it is not
the default because additivity is then not guaranteed.

\texttt{ss\_rcif\_equal\_causes\_test()} is a subject-bootstrap Wald
reconstruction of the equal-cause RCIF test proposed by Sivadasan and
Sankaran in \emph{METRON} (the journal) \citep{sisuma2022}. It compares
integrated contrasts \(F_l(t)-F(t)/k\) across causes. It is a weighted
cumulative-incidence contrast with a bootstrap covariance estimate, not
a log-rank test, and its reconstruction should not be presented as
byte-for-byte author software.

\subsection{Validation against prior
implementations}\label{validation-against-prior-implementations}

Each row compares the same event-time grid and survival values with the
indicated installed package \citep{survrec, newtestsurvrec}.
...
Luo-Huang simulation tables \citep{wang1999, luo2011} remains a separate
validation task requiring a matched design and transcribed published
benchmarks. Zhao's results below assess a different rank-test
construction and cannot fill that role. The existing K-group
polysomnography (PSG) simulation is an application-specific extension
check, not a reproduction of Luo-Huang's two-group study.
...
log-rank, Peto-Prentice, and Gehan-Breslow weights. OLR and Jung-Jeong
columns are not used as benchmarks for this implementation because the
manuscript evaluates the three Zhao extended tests, not the ordinary
unadjusted log-rank or the separate Jung-Jeong clustered statistic.
Those columns remain useful contextual comparators in the source paper
but would answer a different validation question; they are not being
claimed as reproduced by \texttt{zhao\_rank\_test()}.
...
\(\exp(-4)\), Weibull shape 2 and scale \(\exp(4)\), log-normal
parameters \((\mu,\sigma)=(4,0.5)\), and log-logistic survival
\(S(t)=\{1+[\exp(-4)t]^2\}^{-1}\). The study-specific generator uses
...
The apparent Weibull parameter difference is only a parameterization
issue: Zhao's Table 1 writes \(S(t)=\exp(-\lambda t^p)\) with \(p=2\)
and \(\lambda=\exp(-8)\). Since an R Weibull distribution uses
\(S(t)=\exp\{-(t/\text{scale})^p\}\), the corresponding scale is
\(\lambda^{-1/p}=\exp(4)\), exactly the value used by the generator. The
exponential implementation uses rate \(\exp(-4)\), so its mean is
\(\exp(4)\); the log-normal uses \texttt{meanlog\ =\ 4}, and the
corrected log-logistic inverse draw uses scale \(\exp(4)\) and shape
parameter \(0.5\). A 200,000-draw source-level sanity check gives means
approximately 54.6, 48.4, 61.9, and 85.5 days for the four
distributions, respectively, consistent with the paper's stated
approximate means 55, 48, 62, and 86. The remaining differences from
Zhao's published simulations are therefore design differences (notably
the \texttt{survsim} generator and incompletely specified censoring
mechanism), not an unintended \texttt{exp(4)}/\texttt{exp(-4)} scale
inversion.
...
At this render, \textbf{108 of 120 cells} are available, of which 108
...
reproducibility artifacts, but are not presented as a scientific table
in the manuscript. They are execution metadata rather than results for
readers.
...
Monte Carlo standard error is \(\{\hat p(1-\hat p)/B\}^{1/2}\).
Rejection-rate intervals use the Wilson score interval: with
\(z=z_{1-\alpha/2}\), center \((\hat p+z^2/(2B))/(1+z^2/B)\) and
half-width \(z\{\hat p(1-\hat p)/B+z^2/(4B^2)\}^{1/2}/(1+z^2/B)\). For
empirical SDs in Table 1 we use the normal-theory approximation
...
Among the 108 comparable completed cells, \textbf{79 are discrepant}
under the criterion above.

Across 72 available power cells, present minus published power ranges
from \textbf{+0.46 to +33.44 percentage points}. These results require
...
\pandocbounded{\includegraphics[keepaspectratio]{jss-manuscript_files/figure-pdf/fig-zhao-comparison-1.pdf}}

}

\caption{\label{fig-zhao-comparison}Published versus study simulation
summaries. The dashed line is equality. Bars show simulation 95\% Monte
Carlo intervals (normal approximation for SDs, Wilson intervals for
rejection rates). Paper uncertainty and rounding also enter the
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(}\FunctionTok{manuscript\_zhao\_table}\NormalTok{(zhao, }\StringTok{"table1"}\NormalTok{), }\AttributeTok{booktabs =} \ConstantTok{TRUE}\NormalTok{,}
             \AttributeTok{align =} \FunctionTok{c}\NormalTok{(}\StringTok{"l"}\NormalTok{, }\StringTok{"l"}\NormalTok{, }\FunctionTok{rep}\NormalTok{(}\StringTok{"r"}\NormalTok{, }\DecValTok{6}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.2069}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.0920}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1839}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.0920}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1839}}@{}}

\caption{\label{tbl-zhao-null-sd}Comparison with Zhao Table 1: empirical
SD of the standardized null statistic at medium heterogeneity. LR =
log-rank, PP = Peto-Prentice, and GB = Gehan-Breslow. Parentheses
contain simulation Monte Carlo SEs; an asterisk marks a discrepancy.}
...
& 1.0020 (0.0022) \\
loglogistic & 1.009 & Pending & 0.997 & 0.9997 (0.0022) & 0.998 &
Pending \\
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(}\FunctionTok{manuscript\_zhao\_table}\NormalTok{(zhao, }\StringTok{"table2"}\NormalTok{), }\AttributeTok{booktabs =} \ConstantTok{TRUE}\NormalTok{,}
             \AttributeTok{align =} \FunctionTok{c}\NormalTok{(}\StringTok{"l"}\NormalTok{, }\StringTok{"l"}\NormalTok{, }\FunctionTok{rep}\NormalTok{(}\StringTok{"r"}\NormalTok{, }\DecValTok{6}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.0792}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1782}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.0792}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1584}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.0792}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.1584}}@{}}

\caption{\label{tbl-zhao-type1}Comparison with Zhao Table 2: one-sided
type-I error probabilities. LR = log-rank, PP = Peto-Prentice, and GB =
Gehan-Breslow. Published integers are divided by 1000 as explained in
the text. Parentheses contain simulation Monte Carlo SEs; an asterisk
marks a discrepancy.}
...
& 0.025 & 0.0257 (0.0005) \\
medium & loglogistic & 0.026 & 0.0275 (0.0005) & 0.025 & Pending & 0.025
& Pending \\
...
0.025 & 0.0253 (0.0005) \\
high & loglogistic & 0.025 & Pending & 0.025 & Pending & 0.024 & 0.0250
(0.0005) \\
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(}\FunctionTok{manuscript\_zhao\_table}\NormalTok{(zhao, }\StringTok{"table3"}\NormalTok{), }\AttributeTok{booktabs =} \ConstantTok{TRUE}\NormalTok{,}
             \AttributeTok{align =} \FunctionTok{c}\NormalTok{(}\StringTok{"l"}\NormalTok{, }\StringTok{"l"}\NormalTok{, }\StringTok{"r"}\NormalTok{, }\FunctionTok{rep}\NormalTok{(}\StringTok{"r"}\NormalTok{, }\DecValTok{6}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.1500}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0667}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.1500}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0667}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.1500}}@{}}
...
power probabilities. LR = log-rank, PP = Peto-Prentice, and GB =
Gehan-Breslow. Parentheses contain simulation Monte Carlo SEs; an
...
low & loglogistic & 0.25 & 0.325 & 0.3898 (0.0049) * & 0.334 & 0.4375
(0.0050) * & 0.334 & 0.4448 (0.0050) * \\
low & loglogistic & 0.50 & 0.828 & 0.9003 (0.0030) * & 0.846 & 0.9396
(0.0024) * & 0.852 & 0.9430 (0.0023) * \\
...
medium & loglogistic & 0.25 & 0.242 & 0.2954 (0.0046) * & 0.247 & 0.3174
(0.0047) * & 0.230 & 0.3086 (0.0046) * \\
medium & loglogistic & 0.50 & 0.695 & 0.7856 (0.0041) * & 0.711 & 0.8162
(0.0039) * & 0.688 & 0.7989 (0.0040) * \\
...
high & weibull & 0.50 & 0.710 & 0.9273 (0.0026) * & 0.683 & 0.8763
(0.0033) * & 0.574 & 0.8399 (0.0037) * \\
...
high & loglogistic & 0.25 & 0.210 & 0.2705 (0.0044) * & 0.212 & 0.2647
(0.0044) * & 0.191 & 0.2533 (0.0043) * \\
high & loglogistic & 0.50 & 0.614 & 0.7315 (0.0044) * & 0.617 & 0.7378
(0.0044) * & 0.569 & 0.7057 (0.0046) * \\
...
\subsubsection{Polysomnography-bout simulation
study}\label{polysomnography-bout-simulation-study}

Polysomnography (PSG) records sleep in scored 30-second epochs. For a
selected sleep stage, consecutive epochs assigned that stage can be
run-length encoded into bouts, and the gaps between bouts form recurrent
gap times. The supplement simulation is designed to resemble that
structure, with: (i) subject frailty, (ii) visit effects, (iii)
persistent burst/recovery states, (iv) 360--480 minute recordings with
terminal censoring, (v) unequal independent-arm sizes (30 control, 45
treatment), and (vi) paired visits with shared participant frailty.
Under the null, treatment and control gaps have the same distribution.
The corresponding PSG-bout data should not be assumed to satisfy the IID
gap assumption required for a renewal interpretation of PSH; stage
dependence, subject frailty, and bout-state persistence are precisely
reasons to prefer the Wang-Chang estimand and clustered inference there.
Power alternatives multiply treatment gaps by 1.25 (longer bouts) or
0.80 (shorter bouts). The script evaluates \(G_0^*\), \(G_1^*\), Zhao
pooled-risk sensitivity analyses, and paired label permutations where
appropriate.
...
\subsubsection{What must be run before a complete manuscript
render}\label{what-must-be-run-before-a-complete-manuscript-render}

The ordinary examples, estimator/reference checks, table formatting, and
manuscript rendering are local and do not launch simulations. The
remaining optional outputs have different costs:
...
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Input needed by render
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Zhao Tables 1--3 & \texttt{data-raw/zhao-2020-table-simulations.R} via
\texttt{zhao-2020-table-simulations.sbatch} & HPC. Each of 120 tasks
contains 100,000 null or 10,000 power replicates; use the corrected
log-logistic tasks plus any other missing task IDs. & One task-specific
RDS per task in \texttt{data-raw/zhao-2020-table-simulations/}; then
render locally. \\
PSG-bout study & \texttt{data-raw/psg-validation-simulations.R} via its
Slurm array & HPC for the planned 10,000 replicates and 999 paired
permutations. Combine task RDS/CSV files into
\texttt{data-raw/psg-validation-results.rds}. & The combined RDS named
above. \\
K-group pilot/validation &
\texttt{data-raw/wc-logrank-k-validation-simulation.R} via its Slurm
array & Local for a small pilot; HPC for 20,000 replicates. Combine task
outputs before treating it as a full validation. &
\texttt{data-raw/wc-logrank-k-validation-results.csv} (the current file
is a pilot). \\
Curve-interval coverage &
\texttt{data-raw/curve-ci-coverage-simulation.R} & HPC recommended:
25,000-subject truth plus 1,000 data sets and 999 bootstraps. Verify
task partitioning/aggregation before submitting the current array
wrapper. & \texttt{data-raw/curve-ci-coverage-results.rds} (not
currently available). \\
Zhao report aggregation &
\texttt{data-raw/zhao-2020-validation-report.R} & Local after Zhao RDS
files arrive; it does not simulate. & Existing task RDS files. \\
Final manuscript artifacts & \texttt{data-raw/reproduce-manuscript.R} &
Local; runs Quarto and writes public tables plus internal metadata. &
Any optional combined outputs supplied above. \\
\end{longtable}
}

At present, the Zhao directory is incomplete. The missing task IDs are
\texttt{4,\ 8,\ 12,\ 16,\ 20,\ 21,\ 24,\ 28,\ 32,\ 33,\ 36,\ 37,\ 40,\ 44,\ 48,\ 52,\ 56,\ 60,\ 64,\ 68,\ 72,\ 76,\ 80,\ 84,\ 88,\ 92,\ 94,\ 96,\ 100,\ 104,\ 108,\ 112,\ 116,\ 120}.
IDs divisible by four are the corrected log-logistic cells; the other
missing IDs should be rerun with the same driver as their original cell.
The validation report and manuscript intentionally show these as pending
